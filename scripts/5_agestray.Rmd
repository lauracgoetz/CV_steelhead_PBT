...
ggsave(plot = age_yr_gg, file = "../final_figures/F3_ageyr.pdf", 
...
ggsave(plot = bins_gg, file = "../final_figures/F4_agebn.pdf", 
...
density_bins <- final_beens %>% 
  ...
  distinct() %>% 
  group_by(PopName, day_bin, kid_sex) %>% 
  mutate(count_total = sum(count_age)) %>% 
  ungroup() %>% 
  distinct()
...
density_gg <- ggplot(supptable_bins %>% filter(age_spawn <5))+
  geom_density(aes(x = day_bin, y=..density.., stat="density", fill = as.factor(age_spawn)), alpha=0.7)+
  scale_fill_discrete(limits=c(4,3,2))+
  facet_grid(kid_sex~PopName)+
  theme_classic()+
  scale_fill_manual(values = c("#39a5c2","#7ee76c","#fff995"))+
...
ggsave(plot=density_gg, file="../figures/Fig3_density.png", width = 20, height = 10, units = "cm", dpi = 800)
```
...
ggsave(plot = fig1add, file = "../final_figures/S1_spwnsn.pdf", 
